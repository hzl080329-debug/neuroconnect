  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="flex gap-3 mb-6">
        ...
        <button onClick={() => doSearch(query)} className="text-[#5B9CF5] font-semibold px-2">{t('search.search')}</button>
      </div>
      {loading ? <p className="text-center text-gray-400 py-10">{t('search.searching')}</p> :
       searched ? <>
        <p className="text-sm text-gray-500 mb-4">{results.length} {t('search.results')}</p>
        ...
        )) : <p className="text-center text-gray-400 py-10">{t('search.noResults')}</p>}
      </> : null}
    </div>
  );
}
